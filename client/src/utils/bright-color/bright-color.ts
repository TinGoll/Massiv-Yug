...
const brightColor = (hex:string , coefficient:number): string => {
    ...
    r = coefficient>0 ? r + Math.round((255-r)*coefficient) : coefficient < 0 ? r - Math.round((r)*Math.abs(coefficient)) : r
    g = coefficient>0 ? g + Math.round((255-g)*coefficient) : coefficient < 0 ? g - Math.round((g)*Math.abs(coefficient)) : g
    b = coefficient>0 ? b + Math.round((255-b)*coefficient) : coefficient < 0 ? b - Math.round((b)*Math.abs(coefficient)) : b  
